...
struct NetworkService: PushNotificationsNetworkable {
    ...
    func register(deviceToken: Data, appId: String, completion: @escaping CompletionHandler) {
        let deviceTokenString = deviceToken.hexadecimalRepresentation()
        let bundleIdentifier = Bundle.main.bundleIdentifier ?? ""

        let metadata = Metadata.update()

        guard let body = try? Register(token: deviceTokenString, appId: appId, bundleIdentifier: bundleIdentifier, metadata: metadata).encode() else { return }
    ...
    private func setRequest(url: URL, httpMethod: HTTPMethod, body: Data? = nil) -> URLRequest {
        ...
        request.addValue("push-notifications-swift \(SDK.version)", forHTTPHeaderField: "X-eventflit-Library")
